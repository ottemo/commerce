...
func restLogin(params *api.StructAPIHandlerParams) (interface{}, error) {
	...
	if params.Request.Method == "GET" && utils.KeysInMapAndNotBlank(params.RequestGETParams, "login", "password") {
		...
		if !utils.KeysInMapAndNotBlank(reqData, "login", "password") {
			return nil, env.ErrorNew(ConstErrorModule, ConstErrorLevel, "fee28a56adb144b9a0e21c9be6bd6fdb", "login and password should be specified")
		}
	...
	return nil, env.ErrorNew(ConstErrorModule, ConstErrorLevel, "68546aa8a6be4c31ac44ea4278dfbdb0", "wrong login or password")
}
