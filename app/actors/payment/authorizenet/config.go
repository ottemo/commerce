	...
	if config := env.GetConfig(); config != nil {
		...
		config.RegisterItem(env.StructConfigItem{
			...
			if utils.CheckIsBlank(value) {
				return nil, env.ErrorNew(ConstErrorModule, ConstErrorLevel, "e3e1002e02da45dd876a2ffd1d7790d2", "can't be blank")
			}
			return value, nil
		})
		...
		config.RegisterItem(env.StructConfigItem{
			...
			if !utils.IsAmongStr(stringValue, ConstDPMActionAuthorizeOnly, ConstDPMActionAuthorizeAndCapture) {
				return nil, env.ErrorNew(ConstErrorModule, ConstErrorLevel, "9164550937f34addb6922dafac332bd1", "should be "+ConstDPMActionAuthorizeOnly+" or "+ConstDPMActionAuthorizeAndCapture)
			}
			return value, nil
		})
