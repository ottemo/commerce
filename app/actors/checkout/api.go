...
func checkoutObtainAddress(params *api.StructAPIHandlerParams) (visitor.InterfaceVisitorAddress, error) {
	...
	if addressID, present := reqData["id"]; present {
		...
		if visitorAddress.GetVisitorID() != currentVisitorID {
			return nil, env.ErrorNew(ConstErrorModule, ConstErrorLevel, "bef277144ac54705b59a47c8e0bc5aa4", "wrong address id")
		}
...
func restCheckoutSetPaymentMethod(params *api.StructAPIHandlerParams) (interface{}, error) {
	...
	for _, paymentMethod := range checkout.GetRegisteredPaymentMethods() {
		if paymentMethod.GetCode() == params.RequestURLParams["method"] {
			...
			return nil, env.ErrorNew(ConstErrorModule, ConstErrorLevel, "bd07849e87894316924c9c754efbc348", "payment method not allowed")
		}
	}

	return nil, env.ErrorNew(ConstErrorModule, ConstErrorLevel, "b8384a4788064a5490fccccb5e958b4e", "payment method not found")
}
	...
	for _, shippingMethod := range checkout.GetRegisteredShippingMethods() {
		if shippingMethod.GetCode() == params.RequestURLParams["method"] {
			if shippingMethod.IsAllowed(currentCheckout) {
				...
				return nil, env.ErrorNew(ConstErrorModule, ConstErrorLevel, "d7fb6ff2b914467bbf56b8d2bea472ef", "shipping method not allowed")
			}
		}
	}

	return nil, env.ErrorNew(ConstErrorModule, ConstErrorLevel, "279a645c6a0344de95c02651a51440fa", "shipping method and/or rate were not found")
}
